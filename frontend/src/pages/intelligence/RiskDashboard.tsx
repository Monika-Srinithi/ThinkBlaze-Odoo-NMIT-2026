      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          <h1 style={{ margin: 0, fontSize: '2.2rem', fontWeight: 800, display: 'flex', alignItems: 'center', gap: '0.75rem', fontFamily: 'var(--font-heading)' }}>
            <AlertTriangle color="var(--accent-rose)" size={34} /> Workforce Risk Engine
          </h1>
            ...
            <div key={r.team} className={isHigh ? 'glass-panel-glow' : 'glass-panel'} style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
              <div>
                ...
                <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.5rem', marginBottom: '1rem' }}>
                  <div style={{ fontSize: '2.6rem', fontWeight: 900, color: isHigh ? 'var(--accent-rose)' : 'var(--primary)', fontFamily: 'var(--font-heading)' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
                    {(r.factors || []).slice(0, 4).map((f: any, i: number) => (
                      <div key={i} style={{ fontSize: '0.825rem', display: 'flex', alignItems: 'center', gap: '0.4rem', color: 'var(--text-primary)' }}>
                        <span style={{ color: isHigh ? 'var(--accent-rose)' : 'var(--primary)' }}>•</span> {f.factor} (+{f.score} pts)
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                  {(currentEvidence.risk_factors || []).map((f: any, i: number) => (
                    <div key={i} style={{ background: 'rgba(0,0,0,0.3)', padding: '0.85rem 1rem', borderRadius: '0.6rem', borderLeft: '4px solid var(--accent-rose)', fontSize: '0.875rem' }}>
              ...
              <div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                  {(currentEvidence.leave_evidence || []).map((l: any, i: number) => (
                    <div key={i} style={{ background: 'rgba(255,255,255,0.03)', padding: '0.75rem 1rem', borderRadius: '0.6rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <div>
